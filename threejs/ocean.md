...
2. 让其随着`sin()`动起来，Y轴上的坐标根据X的值变化
   ...
   <img src=".\images\image-20221205103904969.png" alt="image-20221205103904969" style="zoom:67%;" />
...
6. 我们可以声明海浪的颜色变量浪高的地方颜色浅一些，浪低的地方颜色深一些，并用debug来控制他
   ...
   - 我们需要获得海浪的翻滚的强度也就是之前声明的`elevation`，并要将其传给`fragment`文件来使用
   ...
   - 在`fragment`文件中声明之前的两个颜色变量，并接收`vElevation`用`mix()`函数来实现海浪颜色的变化
   ...
   - 我们可以加上`mixStrength`来控制混合颜色的强度
     ...
     ![image-20221205152100584](.\images\image-20221205152100584.png)

7. 我们可以用之前学的柏林噪音来制造些小波浪

   ![image-20221205152152032](.\images\image-20221205152152032.png)
   ...
   - 我们在`vertex`文件中使用它
     ...
     ![image-20221205160127638](.\images\image-20221205160127638.png)

     - 一般浪没有那么平滑会有一些棱角和凹凸的地方，我们对其取绝对值再相减

       ```glsl
       elevation -= abs(cnoise(vec3(modelPosition.xz * 3.0, uTime * 0.2)) * 0.15);
       ```

     ![image-20221205160720041](.\images\image-20221205160720041.png)
     ...
     ![image-20221205161225768](.\images\image-20221205161225768.png)

     - 但这些波浪的浪高很奇怪呈现出许多倒刺的感觉，我们需要用`i`来减弱它，我们需要降低最外层的浪高
     ...
     ![image-20221205162803237](.\images\image-20221205162803237.png)
     ...
     ![image-20221205163250433](.\images\image-20221205163250433.png)

   - 我们将这些固定的值变成可以自定义调节的值
...
![image-20221205165123809](.\images\image-20221205165123809.png)
